...
const UA = 'TheMarket/1.0 (info@the-market.co.il)';

const CATEGORIES = ['שוק ההון', 'כלכלה', 'טכנולוגיה', 'נדל"ן', 'צרכנות'];
const AUTHORS = ['רון אלקלעי', 'מיכל ברגר', 'יואב שטרן', 'שירה כהן', 'דניאל פרץ', 'מערכת The Market'];
...
const FALLBACK_QUERIES = {
  'שוק ההון': ['Tel Aviv Stock Exchange', 'stock market trading', 'financial charts screen'],
  'כלכלה': ['Bank of Israel', 'Israeli shekel money', 'economy business city'],
  'טכנולוגיה': ['technology startup office', 'data center servers', 'smartphone technology'],
  'נדל"ן': ['Tel Aviv skyline buildings', 'apartment construction crane', 'real estate house'],
  'צרכנות': ['supermarket shopping', 'shopping mall retail', 'credit card payment'],
};
...
const ARTICLE_SCHEMA = {
  ...
  properties: {
    ...
    coverImageQuery: { type: 'string', description: 'ביטוי חיפוש באנגלית לתמונת נושא אמיתית. העדף ישות מוכרת (למשל "Tel Aviv Stock Exchange", "Bank of Israel", "Nvidia", "Bitcoin") או סצנה כללית רלוונטית ("stock market trading floor", "apartment buildings construction").' },
...
const SYSTEM = `אתה עורך וכתב כלכלה בכיר ומקצועי במגזין "The Market" — מגזין כלכלה ועסקים ישראלי בסגנון "דה מרקר". אתה כותב עברית עיתונאית-כלכלית תקנית, חדה וזורמת.

עקרונות כתיבה:
- כתוב בעברית תקנית, מקצועית ומדויקת, בסגנון עיתונות כלכלית ישראלית רצינית. אל תמציא נתונים/מחירים/שערים ספציפיים כעובדות ודאיות — נסח כמגמות, טווחים או הערכות ("סביב", "כ-", "בשנה החולפת").
- **עברית נקייה בכל הטקסט (קריטי):** כתוב כל מילה עברית באותיות עבריות בלבד. **לעולם אל תמזג אותיות לטיניות בתוך מילה עברית**. בכותרות ובכותרות המשנה אל תשלב ראשי-תיבות/מילים באנגלית כלל (כתוב "בינה מלאכותית" לא "AI", "היי-טק" לא "hi-tech"). שמות חברות זרות — כתוב בתעתיק עברי (למשל "אנבידיה", "אפל", "גוגל"). בדוק שכל מילה תקינה.
- הכותרת קצרה, חדה ותקנית דקדוקית, בסגנון כותרת כלכלית. בדוק שהיא נקראת חלק בעברית.
- מבנה: פסקת פתיחה (לִיד) שעונה ישירות על השאלה/מגמה המרכזית, ואז כותרות משנה (##) הגיוניות, לעיתים ### תת-כותרות.
- העדף להזכיר ישויות מוכרות (הבורסה בתל אביב, בנק ישראל, מדד ת"א 35, נאסד"ק, חברות כמו אנבידיה/אפל/טבע, שער הדולר-שקל) — כדי שניתן יהיה למצוא תמונות אמיתיות ולחזק את האמינות.
...
- כותרות סמנטיות, רשימות, הדגשות של מונחים.
- כלול **טבלת נתונים/השוואה אחת** בפורמט Markdown (| עמודה | עמודה |) הרלוונטית לנושא (למשל השוואת אפיקי השקעה, מדדים, או חברות).
...
- הזכר ישויות מפורשות (חברות, מדדים, מוסדות).

שבץ 1-2 מצייני תמונה בגוף הכתבה, כל אחד בשורה נפרדת: [IMAGE: english search query] (למשל [IMAGE: Tel Aviv Stock Exchange building]).
אורך הכתבה: כ-700-1000 מילים.`;

async function generateArticle(category, avoidTitles) {
  const prompt = `כתוב כתבה חדשה ומקורית בקטגוריית "${category}" למגזין הכלכלה The Market.
בחר נושא ספציפי, רלוונטי ואקטואלי לקהל הישראלי (מגמה בשוק ההון, מהלך כלכלי, חברת טכנולוגיה, שוק הנדל"ן, או נושא צרכני) — שאינו חופף לכותרות הקיימות הבאות:
